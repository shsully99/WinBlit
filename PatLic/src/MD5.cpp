...
{
	TCHAR tchA [] = {0x57, 0x69, 0x6E, 0x42, 0x6C, 0x69, 0x74, 0x50, 0x72, 0x6F, 0x6F, 0x66, 0x52, 0x65, 0x61, 0x64 , 0x0};
	TCHAR tchB [] = {0x50, 0x61, 0x74, 0x61, 0x53, 0x75, 0x63, 0x69, 0x61, 0x6C, 0x74, 0x64 , 0x0};
  ...
  //modify the RFC 1321 code to take 'const' plaintext, see example below. 
	m_plainText = new TCHAR [512];
	int iA = 0; 
	// Copy over WinBlitProofRead
	memcpy (m_plainText, tchA, 16); 
	iA += 16;

	// Copy over non-whitepsace chars from the text string
	for (unsigned int iB = 0; iB < strlen(plainText); iB++)
	{
		if (plainText[iB] != ' ')
		{
			m_plainText[iA] = plainText[iB];
			iA++;
		}
	}

	// Copy over PataSucialtd 
	memcpy ((void *) &m_plainText[iA], tchB, 12);
...
bool CMD5::calcDigest()
{
  ...
  for (int i = 0; i<16; i++)
  {
	wsprintf(&m_digestString[p],"%02x", m_digest[i]);
	p+=2;
  }
  return true;
}
...
bool CMD5::ValidatePAK (const char * tchKey, const char * tchPAK )
{
	bool bRet = false;
	TCHAR tchPAKVal [25];
	unsigned int iA = 0, iB = 0; 
	while (iA < strlen(tchPAK) && iB < 25)
	{
		if (tchPAK[iA] != '-' )
		{
			tchPAKVal[iB] = tchPAK[iA];
			iB++;
		}
		iA++;
	}
	...
	CharUpperBuff(m_digestString, 32); 
	CharUpperBuff(tchPAKVal, strlen(tchPAKVal)); 

	if ( 0 == strncmp(m_digestString, tchPAKVal,25))
	{
		bRet = true; 
	}
	else
