...
//			lstrcpyW(pdbi->wszTitle, L"WinBlit Comm Band");
			...
			{
				bstrTemp.Append(cbstrLicStat[1+(m_iLic/ciLicMultiplier)]);
			}
...
void CWinBlitExplorer::LoadDocument (IDispatch *pDispatch)
{
	...
	if (bSelection)
	{
		hr = m_WinBlitWindow->LoadData(eCalledfromWebBand + m_iLic + 0, varText); 
	}
	else
	{
		hr = m_WinBlitWindow->LoadData(eCalledfromWebBand + m_iLic + 5, varDocument); 
	}

	if (SUCCEEDED(hr))
	{
		m_WinBlitWindow->Display(); 
		// Commented out 14/4/3 as there is now a new autostart flag. 
